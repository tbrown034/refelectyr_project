// app/(media)/movies/error.js
"use client"; // Error boundaries must be Client Components

import { useEffect } from "react";
import Link from "next/link";
import { HomeIcon, FilmIcon } from "@heroicons/react/24/solid";

export default function MoviesError({ error, reset }) {
  useEffect(() => {
    // Log the error to an error reporting service
    console.error("Movies section error caught:", error);
  }, [error]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
        <div className="flex flex-col items-center">
          <div className="bg-red-100 dark:bg-red-900/30 p-4 rounded-full mb-6">
            <FilmIcon className="h-12 w-12 text-red-600 dark:text-red-400" />
          </div>

          <h1 className="text-2xl font-bold mb-4">Unable to Load Movies</h1>
          <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
            We're sorry, but we encountered an error while loading the movies
